@push('css')
<style>
    /*.select2-result-content { padding-top: 2px; padding-bottom: 2px; }*/
    .select2-result-content { padding: 0px; }
    .select2-result__avatar { float: left; width: 60px; margin-right: 5px; }
    .select2-result__avatar img { width: 100%; height: auto; border-radius: 2px; }
    .select2-result__meta { margin-left: 65px; }
    .select2-result__nombre, .select2-result__laboratorio { color: black; font-weight: bold; word-wrap: break-word; line-height: 1; font-size: 12px}
    .select2-result__nombre { margin-bottom: 2px;}
    .select2-result__precio { margin-right: 1em; }
    .select2-result__precio, .select2-result__ubicacion {
        font-weight: bold; margin-bottom: 0px; margin-top: 0px; display: inline-block; color: #01ec51; font-size: 14px; padding: 0px;
    }
    .select2-result__description { color: #777; word-wrap: break-word; line-height: 1; font-size: 12px}
    .select2-results__option--highlighted .select2-result__nombre { color: white; }
    .select2-results__option--highlighted .select2-result__precio,
    .select2-results__option--highlighted .select2-result__ubicacion { color: #01ec51; }
    .select2-results__option--highlighted .select2-result__description { color: #c6dcef; }
    .select2-results__option{ padding: 2px; }
</style>
@endpush
<div class='select2-result-content clearfix'>"+
    @if($imagen=='undefined')
        "<div class='select2-result__avatar'><img src='{!! asset($imagen) !!}' class='img-responsive' alt='Image'></div>"+
    @else
        "<div class='select2-result__avatar'><img src='{!! asset('img/avatar_none.png') !!}' class='img-responsive' alt='Image'></div>"+
    @endif
    "<div class='select2-result__meta'>"+
        "<div class='select2-result__nombre'>{{$nombre or 'nombre'}} / {{$descripcion or 'descripcion'}}</div>"+
        "<div class='select2-result__laboratorio'>{{$laboratorio or ''}}</div>"+
        "<div class='select2-result__precio'><i class='fa fa-money'></i> Q {{$precio}} </div>"+
        "<div class='select2-result__ubicacion'><i class='fa fa-archive'></i> {{$ubicacion or ''}} </div>"+
        "<div class='select2-result__description'>{{$contiene or ''}}</div>"+
    "</div>"+
"</div>